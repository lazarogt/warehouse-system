...
- El sistema funciona offline y ya no depende de PostgreSQL.
- Los flujos que cambian stock real (`movements`, `transfers`, `dispatches`, `adjustments` y conciliaciones de `cycle counts`) se ejecutan con transacciones inmediatas y dejan trazabilidad en `stock_movements` para reconciliar contra `warehouse_stock`.
- La ruta de la base se resuelve en este orden:
- `SQLITE_DB_PATH` si existe
- `Electron userData/database.db` si el backend corre dentro de Electron
- `./data/database.db` como fallback local
- En Docker Compose la base se persiste en el volumen `/data`.
...
- `SQLITE_DB_PATH`: ruta opcional del archivo SQLite. Si no se define, el backend resuelve la ruta automaticamente.
...
- SQLite file: `server/data/database.db` en local o `/data/database.db` en Docker
>>>>>>> fa3551f (Migrate to SQLite)
