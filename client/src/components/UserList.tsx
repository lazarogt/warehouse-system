  ...
  return (
    <section className="panel-surface">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="toolbar-label">{t("users.list")}</p>
          <h3 className="mt-2 text-2xl font-semibold text-white">{t("users.list")}</h3>
          <p className="mt-2 text-sm text-slate-300">{t("users.listSubtitle")}</p>
        </div>
        ...
        <table className="table-fixed w-full min-w-[920px]">
          <thead>
            <tr className="border-b border-white/10 bg-white/[0.04] text-left text-[11px] uppercase tracking-[0.24em] text-slate-500">
              ...
              <th className="px-5 py-4 font-medium">{t("common.email")}</th>
              <th className="px-5 py-4 font-medium">{t("users.role")}</th>
              <th className="px-5 py-4 font-medium">{t("users.status")}</th>
            ...
            {loading && (
              <tr>
                <td colSpan={5} className="px-5 py-6 text-sm text-slate-300">
                  {`${t("loading.section")} ${t("users.list").toLowerCase()}...`}
                </td>
              </tr>
            )}
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-5">
                      <div>
                        <p className="truncate text-sm font-semibold text-white">
                          {safeText(user.name, t("users.noName"))}
                        </p>
                        <p className="mt-1 truncate text-xs text-slate-400">@{safeText(user.username, t("users.noUsername"))}</p>
                        {isCurrentUser && (
                          <p className="mt-1 text-xs uppercase tracking-[0.22em] text-cyan-300">
                            {t("users.sessionCurrent")}
                          </p>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-5 text-sm text-slate-200">
                      <span className="block truncate">{safeText(user.email, t("users.noEmail"))}</span>
                    </td>
                    <td className="px-5 py-5">
                      <span
                    ...
                    <td className="w-40 px-5 py-5 text-right">
                      <div className="flex flex-col items-end gap-2 flex-none">
                        {canManage && (
                          <>
                            <MotionButton
                              aria-label={`${t("users.resetPassword")} ${user.name}`}
                            ...
                            >
                              {resettingUserId === user.id ? t("users.resetting") : t("users.resetPassword")}
                            </MotionButton>
                            <MotionButton
